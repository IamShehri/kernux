...
const cases: Array<[number, string, () => void | Promise<void>]> = [
  ...
  [73, "pullRequestNumber schema and runtime share the positive safe-integer maximum", async () => { const schema=JSON.parse(readFileSync(new URL("../../../schema/o4-read-only-review-product-lineage-evidence.schema.json",import.meta.url),"utf8")); assert.equal(schema.properties.pullRequestNumber.maximum,Number.MAX_SAFE_INTEGER); const f=await fixture(); const input=clone(f.input) as any; input.publicationIntents[0].pullRequestNumber=Number.MAX_SAFE_INTEGER+1; assert.throws(()=>createO4aReadOnlyReviewProductLineageEvidence(input),/bounded safe integer/) }],
  [74, "commentBodyByteLength schema maximum matches the O4-A trigger-comment byte bound", () => { const schema=JSON.parse(readFileSync(new URL("../../../schema/o4-read-only-review-product-lineage-evidence.schema.json",import.meta.url),"utf8")); assert.equal(schema.properties.commentBodyByteLength.maximum,O4A_LIMITS.maxTriggerCommentUtf8Bytes) }],
  [75, "headBindingEvidenceRefs schema preserves canonical O1 non-empty positive binding", () => { const schema=JSON.parse(readFileSync(new URL("../../../schema/o4-read-only-review-product-lineage-evidence.schema.json",import.meta.url),"utf8")); assert.equal(schema.properties.headBindingEvidenceRefs.minItems,1) }],
]
