...
## Current P7-R27 reconciliation candidate

The only active product-status mutation is the documentation-only reconciliation authorized by PR #468 / proof `5587840460`. It is limited exactly to:
...
No sixth path is authorized. This candidate records canonical R26/R27 truth only, preserves unrelated state and predecessor non-grants, and cannot certify its own closure.
...
## Canonical R27 implementation evidence
...
R27 means only deterministic evidence binding over exact canonical R22/R26 lineage plus optional exact P5-R1 provenance and caller-supplied attempt/provider/model/configuration/policy/termination evidence. It does not establish provider invocation, provider authenticity, termination authenticity, external-clock authenticity, retry authority, verified complete review, K5 reconciliation, Done Gate, release, or project completion. `COMPLETED` remains caller-asserted and requires explicit `NOT_TRUNCATED`; non-completed dispositions remain completion debt. The implementation is bounded, deterministic, deeply immutable, data-only, and side-effect-free.
