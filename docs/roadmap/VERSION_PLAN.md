...
Historical authorization, reconciliation, repair, and anomaly records remain authoritative in their dedicated sources even when omitted from this condensed version view.

---

## Active version-plan unit — post-privacy/egress current-view reconciliation candidate
...
No sixth path is authorized. The reconciliation may only bind already-proven truth through PR #524 and must preserve the adverse PR #520 lineage plus the separate PR #522/#523 remediation lineage.
...
The `VERSION = 1` value is the existing `kodac.cli-result` protocol version. It is not package-version, release-version, `--version`, package publication, or public-release authority.
